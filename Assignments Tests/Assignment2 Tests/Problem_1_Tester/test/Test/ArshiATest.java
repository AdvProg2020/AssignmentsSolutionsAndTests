...
public class ArshiATest {
    ...
    private static final int numberOfTests = 9;
    ...
    private boolean checkOutputResults() {
        try {
            ...
            while (outputScanner.hasNextLine() && resultScanner.hasNextLine()) {
                String outputLine = outputScanner.nextLine().trim().replaceAll("kodoon","kodoom");
                String resultLine = resultScanner.nextLine().trim().replaceAll("kodoon","kodoom");
